...
class EnvertechCloud {
    constructor(pAdapter, pUrl, pTimeout) {
        this.adapter = pAdapter;
        this.url = pUrl || 'https://www.envertecportal.com';
        ...
        this.axios.defaults.timeout = pTimeout || 30 * 1000;
    }
     ...
     * getGatewayInfo
     */
    async getGatewayInfo(pStationId) {
        this.adapter.log.debug(`EnvertechCloud.getGatewayInfo (${pStationId})`);
        ...
        try {
            const url = `${this.url}/ApiInverters/QueryTerminalReal?page=1&perPage=20&orderBy=GATEWAYSN&whereCondition=%7B%22STATIONID%22%3A%22${pStationId}%22%7D`;
            this.adapter.log.debug(`request: ${url}`);
            const resp = await this.axios.post(url, {});
            this.adapter.log.debug(`response: status ${resp.status} - ${JSON.stringify(resp.data)}`);
            if (resp.status !== 200) {

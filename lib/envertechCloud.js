...
class EnvertechCloud {
    ...
    async login(pUsername, pPassword) {
...

        {
            ...
            if (this.log) {
                this.adapter.log.info(`[REQUEST ] ${urlMasked}`);
            }
            try {
                const resp = await this.axios.post(url, {});
                if (this.log) {
                    this.adapter.log.info(`[RESPONSE] status ${resp.status} - ${JSON.stringify(resp.data)}`);
                }
                if (resp.status !== 200) {
                    // http error
                ...
                if (this.log) {
                    this.adapter.log.info(`[RESPONSE] error: ${err.toString()}}`);
                }
...

        {
            ...
            if (this.log) {
                this.adapter.log.info(`[REQUEST ] ${url}`);
            }
            try {
